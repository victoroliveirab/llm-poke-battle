...
const envSchema = z
  .object({
    ...
    HOST: z.string().optional().default("127.0.0.1").transform((value) => value.trim()).pipe(z.string().min(1)),
    MCP_PATH: z.string().optional().default("/mcp").transform((value) => value.trim()).pipe(z.string().min(1))
  })
